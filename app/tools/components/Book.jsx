...
function Book({ book, scrollX, index }) {
    ...
    // book details screen
    const bookDetails = () => {
      ReactNativeHapticFeedback.trigger('selection'); // Changed this line
    ...
    const anims = {
      book: useAnimatedStyle(() => ({
        ...
        transform: [
          { perspective: 800 },
          { scale: interpolate(position.value, inputRange, [0.9, 1, 1, 1], 'clamp') },
          { rotateY: `${interpolate(position.value, inputRange, [60, 0, 0, 0], 'clamp')}deg` },
          {
